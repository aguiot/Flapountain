...
namespace glimac {
    ...
    void Flappy::display(VAO &vao, VBO &vbo, glm::mat4 ProjMatrix, glm::mat4 MVMatrix, Texture texture, Uniforms &uniforms, Light light) {
        ...
        glm::vec3 normV = glm::normalize(glm::vec3(velocity[0], 0, velocity[2]));
        float phi = glm::sign(glm::cross(glm::vec3(-1.f, 0.f, 0.f), normV)[1]) * std::acos(glm::dot(glm::vec3(-1.f, 0.f, 0.f), normV));
        glm::vec3 normPartV = glm::normalize(velocity);
        float theta = - glm::sign(glm::cross(normV, normPartV)[2]) * std::acos(glm::dot(normV, normPartV));
    ...
    void Flappy::launch(std::vector<Pipe> &pipes) {
        ...
        velocity = glm::vec3(2.f*Vix, 1.05f * Viy, 2.f*Viz);
    }
